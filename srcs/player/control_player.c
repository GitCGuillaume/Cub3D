...
void	move_forward(t_map *map, double sin_degree, double cos_degree)
{
	...
	{
		if (map->lines[(int)(check_pos_y - sin_degree * 0.35)]
				[(int)check_pos_x] != ' ' && map->lines[(int)(check_pos_y
					- sin_degree * 0.35)][(int)check_pos_x] != '1')
...
int	control_player(void *param)
{
	...
	map = (t_map *)param;
	sin_degree = sin(degree_to_radian(map->player.degree_raycast));
	cos_degree = cos(degree_to_radian(map->player.degree_raycast));
	...
	if (map->control.t_left == 1)
		map->player.degree_raycast =
			correct_distance(map->player.degree_raycast) + 3.0;
